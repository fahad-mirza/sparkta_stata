...
class StatsRenderer {
    ...
    String buildStatsBlock(String heading, List<Variable> vars, int idx, int groupN) {
        ...
        String badge = "<span class='grp-badge'>N=" + groupN + "</span>";
        StringBuilder sb = new StringBuilder();
    ...
    String buildStatsTable(List<Variable> vars, int tblIdx) {
        ...
        for (Variable v : vars) {
            ...
            String cvBadge = "<span class='cv " + cvClass + "'>CV " + cvPct + "%</span>";
            sb.append("          <tr>\n");
            sb.append("            <td class='var-name'>").append(escHtml(v.getDisplayName())).append("</td>\n");
            // N with nb badge
            sb.append("            <td data-col='0'><span class='nb'>").append(n).append("</span></td>\n");
            // Mean
            sb.append("            <td data-col='1' data-v='").append(mean).append("'>").append(fmtStat(mean)).append("</td>\n");
            // Median
            sb.append("            <td data-col='2' data-v='").append(median).append("'>").append(fmtStat(median)).append("</td>\n");
            // Min
            sb.append("            <td data-col='3' data-v='").append(min).append("'>").append(fmtStat(min)).append("</td>\n");
            // Max
            sb.append("            <td data-col='4' data-v='").append(max).append("'>").append(fmtStat(max)).append("</td>\n");
            // Std Dev with CV badge
            sb.append("            <td data-col='5' data-v='").append(sd).append("'>").append(fmtStat(sd)).append(" ").append(cvBadge).append("</td>\n");
            // Distribution sparkline
            sb.append("            <td class='spark-cell' data-col='6'>\n");
            sb.append(buildSparkline(v, min, max, mean, median, q1, q3));
            sb.append("            </td>\n");
            sb.append("          </tr>\n");
        }
        sb.append("          </tbody>\n        </table></div>\n");
        // Footer row: sort-hint on left, sparkline legend on right (under Distribution col)
        // Both wrapped in data-col='6' so they hide when Distribution chip is toggled off
        sb.append("        <div data-col='6' class='stats-footer'>\n");
        sb.append("          <div class='sort-hint'>Click any column header to sort</div>\n");
        sb.append("          <div class='spark-legend'>\n");
