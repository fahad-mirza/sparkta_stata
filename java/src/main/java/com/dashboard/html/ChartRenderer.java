...
class ChartRenderer {
    ...
    String barLine(String id, DataSet data, boolean isLine) {
        ...
        String xScaleCfg = buildAxisConfig("x", xTitleStr, o.chart.horizontal, isBar, isLine);
    ...
    String scatterOverDs(DataSet data, Variable xv, Variable yv) {
        ...
        // Without this, each panel restarts ci=0, so rep78=3 in Foreign gets col(0)
        // instead of col(2). Scatter has no x-axis position issue (each point carries
        // its own {x,y}), but colors must match across panels.
    ...
    /**
     * Builds the datasets[] string for a scatter filter slice. (v2.6.2)
     * Delegates to scatterSingleDs or scatterOverDs depending on whether
     * the slice has an over() variable. Called by FilterRenderer via delegate.
     * Returns the datasets content string (without surrounding brackets).
     * Returns empty string if slice has fewer than 2 numeric variables.
     */
    ...
    String ciLine(String id, DataSet data) {
        ...
        Variable ov = data.getOverVariable();
        List<String> groups = DataSet.uniqueValues(ov, o.chart.sortgroups);
    ...
    String pie(String id, DataSet data, boolean donut) {
        ...
        List<String> groups    = DataSet.uniqueValues(overVar, o.chart.sortgroups);
        List<String> groupKeys = DataSet.uniqueGroupKeys(overVar, o.chart.sortgroups);
        double total=0;
        ...
        String borderColor = gen.isDark()    ? "'#16213e'" : "'#ffffff'";
