/*
 * sparkta_engine.js  v1.8  (F-1, sparkta)
 *
 * Browser-side aggregation engine for sparkta interactive filter system.
 *
 ...
 *
 * v1.8: buildCiBands() extended: exp/log/power CI bands.
 *        exp:   OLS on (x,ln(y)), SE on log scale, exp-transform bounds.
 *        log:   OLS on (ln(x),y), exact symmetric CI (linear model).
 *        power: OLS on (ln(x),ln(y)), SE on log-log scale, exp-transform.
 *
 * v1.7: buildCiBands() fixed for qfit: now uses olsQuad residuals and df=n-3
 *        instead of olsLinear for both fit types. qfit CI band correctly curves
 *        (trumpet shape) on filter/slider changes matching FitComputer.java.
 *
...
(function(w) {
    ...
    function buildCiBands(rows, xVar, yVar, fitType) {
        ...
        if (ft === 'qfit' && n >= 4) {
            // qfit CI: uses exact qfit residuals and fitted values.
            // SE approximation: s * sqrt(1/n + (xi-xbar)^2/Sxx)
            // where s = sqrt(RSS/(n-3)), matching FitComputer.java computeQfit().
            var q = olsQuad(sxs, sys);
            var rssQ = 0;
            for (var iq = 0; iq < n; iq++) {
                var resQ = sys[iq] - (q.a + q.b*sxs[iq] + q.c*sxs[iq]*sxs[iq]);
                rssQ += resQ * resQ;
            }
            var sQ = Math.sqrt(rssQ / (n - 3));
            var xbarQ = 0;
            ...
            // exp CI: OLS on (x, ln(y)), SE on log scale, exp-transform bounds.
            // CI is asymmetric in y-space (wider above fit line than below).
            // Matching FitComputer.java computeExp() with withCi=true.
            var lnys = [], validXe = [];
            for (var ie = 0; ie < n; ie++) {
                if (sys[ie] > 0) { lnys.push(Math.log(sys[ie])); validXe.push(sxs[ie]); }
            }
            if (validXe.length < 3) return null;
            var me = validXe.length;
            var re = olsLinear(validXe, lnys);
            var rssE = 0;
            for (var ie2 = 0; ie2 < me; ie2++) {
                var resE = lnys[ie2] - (re.a + re.b*validXe[ie2]);
                rssE += resE*resE;
            }
            var sE = (me > 2) ? Math.sqrt(rssE/(me-2)) : 0;
            var xbarE = 0;
            for (var ie3 = 0; ie3 < me; ie3++) xbarE += validXe[ie3];
            xbarE /= me;
            var SxxE = 0;
            ...
            for (var ie5 = 0; ie5 < n; ie5++) {
                ...
                var seE = sE * Math.sqrt(1.0/me + (xiE-xbarE)*(xiE-xbarE)/SxxE);
                upper.push({x: xiE, y: Math.exp(lnyiE + tE*seE)});
                lower.push({x: xiE, y: Math.exp(lnyiE - tE*seE)});
            }

        } else if (ft === 'log') {
            // log CI: OLS on (ln(x), y) -- exact linear model, symmetric CI in y.
            // SE = s * sqrt(1/m + (ln(xi)-lnxbar)^2/Sxx_lnx)
            // Matching FitComputer.java computeLog() with withCi=true.
            var lnxsL = [], ysL = [];
            for (var il = 0; il < n; il++) {
                if (sxs[il] > 0) { lnxsL.push(Math.log(sxs[il])); ysL.push(sys[il]); }
            }
            if (lnxsL.length < 3) return null;
            var ml = lnxsL.length;
            var rl = olsLinear(lnxsL, ysL);
            var rssL = 0;
            for (var il2 = 0; il2 < ml; il2++) {
                var resL = ysL[il2] - (rl.a + rl.b*lnxsL[il2]);
                rssL += resL*resL;
            }
            var sL = (ml > 2) ? Math.sqrt(rssL/(ml-2)) : 0;
            var lxbarL = 0;
            for (var il3 = 0; il3 < ml; il3++) lxbarL += lnxsL[il3];
            lxbarL /= ml;
            var SxxL = 0;
            ...
            for (var il5 = 0; il5 < n; il5++) {
                ...
                var yiL = rl.a + rl.b*lnxiL;
                var seL = sL * Math.sqrt(1.0/ml + (lnxiL-lxbarL)*(lnxiL-lxbarL)/SxxL);
                upper.push({x: sxs[il5], y: yiL + tL*seL});
                lower.push({x: sxs[il5], y: yiL - tL*seL});
            }

        } else if (ft === 'power') {
            // power CI: OLS on (ln(x), ln(y)), SE on log-log scale, exp-transform.
            // Asymmetric CI in y-space. Matching FitComputer.java computePower().
            var lnxsP = [], lnysP = [];
            for (var ip = 0; ip < n; ip++) {
                if (sxs[ip] > 0 && sys[ip] > 0) {
                    lnxsP.push(Math.log(sxs[ip])); lnysP.push(Math.log(sys[ip]));
                }
            }
            if (lnxsP.length < 3) return null;
            var mp = lnxsP.length;
            var rp = olsLinear(lnxsP, lnysP);
            var rssP = 0;
            for (var ip2 = 0; ip2 < mp; ip2++) {
                var resP = lnysP[ip2] - (rp.a + rp.b*lnxsP[ip2]);
                rssP += resP*resP;
            }
            var sP = (mp > 2) ? Math.sqrt(rssP/(mp-2)) : 0;
            var lxbarP = 0;
            for (var ip3 = 0; ip3 < mp; ip3++) lxbarP += lnxsP[ip3];
            lxbarP /= mp;
            var SxxP = 0;
            ...
            for (var ip5 = 0; ip5 < n; ip5++) {
                ...
                var lnyiP = rp.a + rp.b*lnxiP;
                var seP = sP * Math.sqrt(1.0/mp + (lnxiP-lxbarP)*(lnxiP-lxbarP)/SxxP);
                upper.push({x: sxs[ip5], y: Math.exp(lnyiP + tP*seP)});
                lower.push({x: sxs[ip5], y: Math.exp(lnyiP - tP*seP)});
            }

        } else {
            // lfit CI (default path -- also fallback for qfit with n<4)
            var reg = olsLinear(sxs, sys);
            var a = reg.a, b = reg.b;
            var rss = 0;
            for (var i3=0; i3<n; i3++){
